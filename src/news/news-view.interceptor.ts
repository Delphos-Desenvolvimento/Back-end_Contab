...
export class NewsViewInterceptor implements NestInterceptor {
    ...
    async intercept(
        ...
        console.log(`[NewsViewInterceptor] Parsed newsId: ${newsId}, sessionId: ${sessionId}`);
        ...
        if (existingView) {
            console.log(`[NewsViewInterceptor] Debounced (DB) for newsId: ${newsId}, sessionId: ${sessionId}`);
        }
        ...
        if (!existingView && !isNaN(newsId)) {
            ...
            console.log(`[NewsViewInterceptor] Skipping view creation. Existing: ${!!existingView}, Valid ID: ${!isNaN(newsId)}`);
        }
